...
# 1) EncoderDecoderCache alias (반드시 가장 위에)
import transformers
from transformers.cache_utils import DynamicCache
transformers.EncoderDecoderCache = DynamicCache
...
def eval_model(model, tokenizer, args):
    ...
    if args.tasks:
        task_names = pattern_match(args.tasks.split(","), tasks.TaskManager().all_tasks)
        lm = HFLM(
            pretrained=args.model_path, # ✅ “문자열 경로” 를 넘깁니다
            backend="causal",
            device="cuda",
            batch_size=args.batch_size,
            tokenizer=tokenizer,
            max_length=max_length,      # 파라미터명도 정확히 max_length 로
        )
